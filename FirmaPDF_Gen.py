...
def _pedirNumeroEntero():
    correcto=False
    num=0
    while(not correcto):
        try:
            num = int(input("Selecciona fichero (pfx) de certificado: "))
            correcto=True
        except ValueError:
            print('Error, introduce un numero del menú')
    return num
...
def _eligefile():
    import os
    import pathlib
    #ejemplo_dir = '/Users/TiiZss/ejemplo/'
    #ejemplo_dir = os.path.dirname(os.path.abspath(__file__))+ "\PoC\static"
    ejemplo_dir = os.path.dirname(os.path.abspath(__file__))+ "\static"
    afile = [""]
    #print (afile[0])
    vopcion = 0
    with os.scandir(ejemplo_dir) as ficheros:
        for fichero in ficheros:
            if fichero.is_file() and fichero.name.endswith('.pfx'):
                vopcion +=1
                print("["+ str(vopcion) +"] " + fichero.name)
                #afile[len(afile):] = str(fichero.name)
                afile.append(fichero.name)
    vvalido = False
    while not vvalido:
        vopcion = _pedirNumeroEntero()
        if vopcion > len(afile):
            vvalido = False
            print ("Solo puedes elegir hasta " + str(len(afile)))
        else:
            print("Has elegido el fichero: " + afile[vopcion])
            vvalido = True
            return afile[vopcion]
...
def create_self_signed_cert(pKey):
    ...
    cert.get_subject().CN = "Tomas Isasia"
    # Serial Number
...
def sign_file2(input_file: str, signatureID: str, x_coordinate: int, 
    ...
    pk_filename = os.path.dirname(os.path.abspath(__file__)) + "\static\\" + _eligefile()
